...
public class ResetScore
{
    ...
    public ResetScore(Plugin plugin)
    {
        ...
        resetDeathFlag = _plugin.Config.ResetDeathFlag is string && _plugin.Config.ResetDeathFlag.Length > 0
            ? _plugin.Config.ResetDeathFlag
            : "@css/general";
    }
    ...
    public void OnResetDeath(CCSPlayerController? player, CommandInfo inf)
    {
        ...
        if (hvh_resetdeath.Value == 1 && !AdminManager.PlayerHasPermissions(player, "@css/general") && !AdminManager.PlayerHasPermissions(player, resetDeathFlag))
            return;
